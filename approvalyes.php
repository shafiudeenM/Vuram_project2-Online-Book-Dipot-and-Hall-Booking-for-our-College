<?php 
     ...
     include "db.php";
         ...
         $date_now  = date("Y-m-d H:i:s");
          ...
          $sql="update hall
          SET STATUS = 'APPROVED',NSTATUS='ALLOWED',DELETS=null
          where HID= $O ";
          $ss=  " insert into hall (STIME)
          values(now())";
...
<link rel="stylesheet" href="css/style.css">
    ...
    <div class=container style="float: right;" >
        <h1 id="header" >VIEW BOOKINGS</h1>
 
     
     <?php
         $sql= " select hall.HALL,hall.HID,hall.NSTATUS,hall.STIME,hall.STATUS,hall.SESSION,hall.COUNT,hall.DEPARTMENT,hall.PURPOSE,hall.TIME,user.UNAME from hall inner join user on hall.UID=user.UID";
          $res=$db->query($sql);
          ...
          if($res->num_rows>0)
          {
              ...
              while($row= $res->fetch_assoc())
              {
                ...
                echo "<td><a href='approvalyes.php?i={$row["HID"]}'  >ACCEPT? </a><hr ><a href='approvalno.php?i={$row["HID"]}'  >DECLINE? </a></td>";
