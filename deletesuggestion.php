...
<link rel="stylesheet" href="css/style.css">
    ...
    <div class=container style="float: right;" >
        <h1 id="header" >VIEW SUGGESTIONS</h1>
 
     
     <?php
         $sql= "select user.UNAME,suggestion.SUID,suggestion.SERVICE,
         suggestion.CONTENT,suggestion.TIME,suggestion.STATUS,suggestion.ETIME,suggestion.DELETS
          from suggestion inner join user on suggestion.UID={$_SESSION["ID"]}";
          $res=$db->query($sql);
              ...
              while($row= $res->fetch_assoc())
              {
                    ...
                    if ($date_now > $row["ETIME"]) {
                     ...
                     echo "<td>$a</td>";
                      ...
                      echo "<td><a href='deletesuggestion.php?i={$row["SUID"]}'>{$row["DELETS"]}</a></td>";
                      }
