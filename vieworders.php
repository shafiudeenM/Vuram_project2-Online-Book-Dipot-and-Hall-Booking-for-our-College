...
<link rel="stylesheet" href="css/style.css">
    ...
    <div class=container style="float: right;" >
     ...
     <?php
      $h= $_SESSION["NAME"];
      echo "$h";
         $sql= " select user.UNAME, dipot.DELETS,dipot.ETIME,dipot.UNAME,dipot.OID,dipot.DEPARTMENT,dipot.REQUIREMENT,dipot.YEAR,dipot.STATUS,dipot.QUANTITY,dipot.TIME FROM 
          dipot inner join user on dipot.UID =  {$_SESSION["ID"]} where User.UNAME= '$h';
          ";
          $res=$db->query($sql);
              ...
              while($row= $res->fetch_assoc())
              {
                    ...
                    if ($date_now > $row["ETIME"]) {
                     ...
                     echo "<td>$a</td>";
                      ...
                      echo "<td><a href='deleteorder.php?i={$row["OID"]}'>{$row["DELETS"]}</a></td>";
                      }
